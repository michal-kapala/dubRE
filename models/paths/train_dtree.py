...
def train_decision_tree(conn: sqlite3.Connection):
  """Trains cross-reference path Decision Tree classifier (scikit-learn) and saves it to a file."""
  cur = conn.cursor()
  start = datetime.now()
  ...
  scores = cross_val_score(tree, X=x_train, y=y_train, scoring='f1')
