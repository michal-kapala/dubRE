...
def train_linear_svc(conn: sqlite3.Connection):
  """Trains cross-reference path Linear Support Vector classifier (scikit-learn) and saves it to a file."""
  cur = conn.cursor()
  start = datetime.now()
  ...
  scores = cross_val_score(svc, X=x_train, y=y_train, scoring='f1')
